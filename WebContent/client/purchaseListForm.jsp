	...
	<div align="center" style="margin-left: 20%; margin-right: 20%;">
		...
		<c:if test="${count>0 }">
			...
			<c:if test="${startPage > pageBlock}">
				<a
					href="/WJElectronics/client/purchase.do?cmd=getPurchaseList&buyerId=<%=session.getAttribute("id") %>&pageNum=${startPage - pageBlock}">[이전]</a>
			</c:if>
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a
					href="/WJElectronics/client/purchase.do?cmd=getPurchaseList&buyerId=<%=session.getAttribute("id") %>&pageNum=${i}">[${i}]</a>
			</c:forEach>
			<c:if test="${endPage < pageCount}">
				<a
					href="/WJElectronics/client/purchase.do?cmd=getPurchaseList&buyerId=<%=session.getAttribute("id") %>&pageNum=${startPage  + pageBlock}">[다음]</a>
			</c:if>
		</c:if>
	</div>
	</div>
